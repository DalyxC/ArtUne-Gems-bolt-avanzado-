...
-- Create artist_media table for portfolio files (if not exists)
...
DROP POLICY IF EXISTS "Artists can view their own media" ON artist_media;
DROP POLICY IF EXISTS "Artists can insert their own media" ON artist_media;
DROP POLICY IF EXISTS "Artists can update their own media" ON artist_media;
DROP POLICY IF EXISTS "Artists can delete their own media" ON artist_media;
DROP POLICY IF EXISTS "Everyone can view media from verified artists" ON artist_media;
DROP POLICY IF EXISTS "Artists can upload to their own folder" ON storage.objects;
DROP POLICY IF EXISTS "Artists can view their own uploads" ON storage.objects;
DROP POLICY IF EXISTS "Everyone can view public portfolio files" ON storage.objects;
...
-- Add trigger for artist_media updated_at if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_artist_media_updated_at'
  ) THEN
